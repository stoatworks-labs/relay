...
Windows `.dll`. MIT. Not yet public, not yet released, never loaded into
Resolume (the fleet's third mixer, after genlock and wipe, which have been).
...
- `StoatworksAbout.h` and `ATTRIBUTIONS.md` are provisional hand copies with
  `guide=""` (no user guide yet, so no User guide button).

## Not done yet
- **Never loaded into Resolume**, on either platform. Everything is measured
  offline through the real plugin class. What genlock and wipe measured in
  Arena (Extra Effects, Blend Mode, padded inputs, SetTime in ms, Opacity
  bound to the layer, parameter 0 hidden) is designed to, not re-measured.
- CI has never run (no remote). The Windows DLL has never been compiled.
